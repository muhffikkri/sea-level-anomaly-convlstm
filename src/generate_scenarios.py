...
# Multi-Step Supervised Sequence Dataset Generator (16x16 Full Spatial Grid)
# Fully Leakage-Safe: Strict Chronological Split BEFORE Sequence Generation
# =========================================================================
...
def create_supervised_sequences(tensor, input_window, output_window):
    ...
    # Batas maksimal iterasi agar tidak melewati batas akhir baris array
    ...
    for start_idx in range(max_start_idx):
        ...
        # Ekstraksi sekuens historis (T) dan target masa depan (H)
    ...
    # Tambahkan dimensi channel (1) di akhir untuk arsitektur ConvLSTM2D (Keras requirement)
    if X.ndim == 4:
...
def generate_all_scenarios():
    ...
    # This isolation completely eradicates look-ahead and overlap temporal leakage.
    # =====================================================================
    ...
    if USE_MINMAX_SCALING:
        ...
        # Ambil hanya nilai finite (abaikan NaN daratan) untuk proses fitting
        train_finite_values = raw_train_tensor[np.isfinite(raw_train_tensor)].reshape(-1, 1)
        scaler.fit(train_finite_values)
        
        # Fungsi internal untuk transformasi aman tanpa merusak NaN landmask
    ...
    for input_window, output_window in tqdm(SCENARIOS, desc="Compiling Scenario Pipelines"):
        ...
        # Pemotongan sliding window mandiri di dalam benteng subset masing-masing
        ...
        # --- SAVE BINARY MATRICES (.NPZ COMPRESSED FORMAT FOR SPEED) ---
        # Menggabungkan file ke dalam satu format .npz terkompresi menghemat ribuan ruang I/O di disk Kaggle
        ...
        # --- SAVE SPECIFIC SCENARIO SCALER ARTIFACT ---
